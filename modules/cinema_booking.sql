...
--
-- Temporary view structure for view `film_seat`
--

DROP TABLE IF EXISTS `film_seat`;
/*!50001 DROP VIEW IF EXISTS `film_seat`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `film_seat` AS SELECT 
 1 AS `filmName`,
 1 AS `sroomNum`,
 1 AS `rowNum`,
 1 AS `colNum`*/;
...
--
-- Final view structure for view `film_seat`
--
...
/*!50001 VIEW `film_seat` AS select `f`.`filmName` AS `filmName`,`se`.`sroomNum` AS `sroomNum`,`se`.`rowNum` AS `rowNum`,`se`.`colNum` AS `colNum` from ((`seat` `se` join `screening` `sc`) join `film` `f`) where ((`f`.`filmId` = `sc`.`sfilmId`) and (`sc`.`room` = `se`.`sroomNum`)) */;
...
-- Dump completed on 2020-05-02 13:18:33
