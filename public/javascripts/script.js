...
$(document).ready(function () {
    ...
    $(document).ready(function () {
        $.ajax({
          ...
          success: function (data) {
              //有三个广告位置
            for (var i = 1; i <= 3; i++) {
                $(".news" + i + " .title").text(data[i-1].title);
                $(".news" + i + " .content").text(data[i-1].content);
            }
          }
        });
    });
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#startTime").append("<option selected disabled style='display:none;'>Choose startTime</option>");

        //去重
        var arrayOfMoviesWithDuplicates = [];
        for (var i = 0; i < data.length; i++) {
            arrayOfMoviesWithDuplicates.push(data[i]);
        };
    ...
    $('#movie').change(function () {
        $.ajax({
          ...
          success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                arrayOfDaysWithDuplicates.push(data[i].day);
            };
    ...
    $('#day').change(function () {
        $.ajax({
          ...
          success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                arrayOfHoursWithDuplicates.push(data[i].startTime);
            };
            ...
            for (var i = 0; i < arrayOfUniqueHours.length; i++) {
                $("#startTime").append("<option value='" + arrayOfUniqueHours[i] + "'>" + arrayOfUniqueHours[i] + "</option>");
            }
        ...
        if (m !== null && d !== null && h !== null) {            
            ...
            for (var k = 0; k <= numberOfRows; k++) {    
                ...
                for (var j = 1; j <= numberOfSeatsInRow; j++) {
                    data += "<div value='" + j + alfabet[k] + "'>" + j +alfabet[k] + "</div>";
                };
                ...
                $.ajax({
                  ...
                  data: {
                    ...
                    for (var i = 0; i < data.length; i++) {
                            ...
                            $('#hall div [value="' + data[i] + '"]').css('background-color', 'red');
                        }
    ...
    function checkDetail() {
        ...
        if (clickedSeat == numberSeatsToReservation) {
            ...
            $("#summationList").append("Movie: <span style='color: #FFF'>" + m + ", " + d + ", " + h +
                             "</span><br/>User: <span style='color: #FFF'>" + name +  ", " + email + ", " + phone +
